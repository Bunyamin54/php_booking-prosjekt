...
<head>
  ...
  <title><?php echo $instillinger_r['site_title']?> Rom </title>
    ...
    <div class="row">
      ...
      <div class="col-lg-9 col-md-12 px-4">
        <div class="horizontal-scroll">
          <div class="card border-0 shadow room-card">
            ...
            <div class="card-body">
              ...
              <div class="vurdering mb-4">
                ...
                <span class="badge rounded-pill bg-light text-success mb-3 text-wrap">
          ...
          <!-- Repeat room card div here for more cards -->
        </div>
      </div>
    </div>

    <?php 
$rom_res = select("SELECT * FROM `rom` WHERE `status`=? AND `removed` =?", [1, 0], 'ii');
while ($rom_data = mysqli_fetch_assoc($rom_res)) {
    
    $fea_q = mysqli_query($con, "SELECT f.name FROM `funksjoner` f 
    INNER JOIN `rom_funksjoner` rfea ON f.id = rfea.funksjoner_id 
    WHERE rfea.rom_id = '{$rom_data['id']}'");
